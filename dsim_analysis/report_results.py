...
def display_sfdr(results):
    ...
    for entry in results:
        ...
        fft_power_spectrum_p0 = np.round(10*np.log10(entry[0][0][0][2]),2)
        fft_power_spectrum_p1 = np.round(10*np.log10(entry[0][0][1][2]),2)
        ...
        plt.plot(fft_power_spectrum_p0, '-D', markevery=markers_p0, markerfacecolor='green', markersize=9)
        plt.plot(fft_power_spectrum_p1, '-D', markevery=markers_p1, markerfacecolor='purple', markersize=9)
        if fundamental_bin_p0 < len(fft_power_spectrum_p0)/2:
            plt.text(8.5e4, 90, f'SFDR Pol0 ($\u25C6$): {difference_dB_p0}dB', color='green', style='italic')
            plt.text(8.5e4, 80, f'SFDR Pol1 ($\u25C6$): {difference_dB_p1}dB', color='purple', style='italic')
        else:
            plt.text(0.25e4, 90, f'SFDR Pol0: ($\u25C6$) {difference_dB_p0}dB', color='green', style='italic')
            plt.text(0.25e4, 80, f'SFDR Pol1: ($\u25C6$) {difference_dB_p1}dB', color='purple', style='italic')
...
def display_freq_step(results):
    for entry in results: # each entry retrieved is all data for a pol
        temp_bin = None
        start_freq = 0
        end_freq = 0
        for test_freq in entry:
            bin = test_freq[0]
            freq = test_freq[1]
            print(f'freq is {freq} in bin {bin}')

            if bin != temp_bin:
                temp_bin = bin

                if start_freq != 0:
                    # update end_freq before computing difference
                    end_freq = freq
                    print(end_freq - start_freq)

                start_freq = freq
            else:
                # update the end_freq as there has been a frequency change but the bin number has not yet incremented.
                end_freq = freq


def display_freq_step_beat(results):
    chunk_samples = results[2]
    ...
    # Note: Take sqrt as results were squred in calculation when detecting bin (cw.py).
    plt.plot(10*np.log10(np.sqrt(results[0][0][0][0:num_chan])), '-D', markevery=markers_p0[0], markerfacecolor='green', markersize=9)
    plt.plot(10*np.log10(np.sqrt(results[0][1][0][0:num_chan])), '-D', markevery=markers_p1[0], markerfacecolor='green', markersize=9)
    plt.title(f'Channelised Data (Channels 0-{num_chan-1})')
    plt.text(5, 67, f'Pol0: ($\u25C6$) {round(results[0][0][1],2)}Hz', color='green', style='italic')
    plt.text(5, 65, f'Pol1: ($\u25C6$) {round(results[0][1][1],2)}Hz', color='purple', style='italic')
